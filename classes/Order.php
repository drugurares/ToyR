	...
	public function getItems() {
		...
		if (!empty($this->_basket)) {
			...
			foreach($this->_basket as $key => $value) {
				$tempPorduct=$objCatalogue->getProduct($key);
				$this->_items[$key] = $tempPorduct;
				}
		}
	...
	private function addItems($order_id = null) {
		if (!empty($order_id)) {
			...
			$error = array();
				...
				if($this->_basket[$item['id']]['qty']>=10)
						$promOk=true;
				...
				$sql="select `stock` from `products` WHERE `id`='".$item['id']."'";
				$res=$this->db->fetchOne($sql);
				$sto=intval($res['stock'])-intval($this->_basket[$item['id']]['qty']);
				$sql="UPDATE `products` SET `stock`='".$sto."' where `id`='".$item['id']."'";
				if (!$this->db->query($sql)) {
					$error[] = $sql;
				}
